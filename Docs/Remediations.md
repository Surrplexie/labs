...
### Open follow-ups (from REM-1 plan, not closed in REM-2–5)

- Confirm **`build_linux.sh`** parity with `build_exe.ps1` (UTC, tkinter note, single pin from `build_requirements.txt`) if not already matched.
- Optional **`build_exe.ps1 -Clean`** post-build temp cleanup.
- Optional **CI smoke** of `workflow_gui` import on PRs (heavy; was deferred).
- Rebuild and tag **`workflow_gui.exe`** after v3.1.0 GUI changes when ready for release (`RELEASE-TAGGING.md`).

### Quick reference — files touched across REM-2–5
...
| GUI | `30_scripts/workflow_gui.py` (v3.1.0) |
